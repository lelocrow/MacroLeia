...
- Each macro can currently contain up to 100 cards.
...
7. Pasting an image into a text field while editing converts that card into an image card and loads the pasted image.
8. In the detail screen, each button shows its sequence number plus a truncated text preview or image thumbnail.
...
Each macro accepts up to 100 items in `buttons`.
