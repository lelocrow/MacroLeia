...
- Resized preview buttons and editor text boxes are saved locally in the browser and stay attached to the same message when texts are reordered.
...
4. A macro can contain one or more text or image cards:
   - If it has one filled card, clicking the macro copies it immediately.
   - If it has multiple filled cards, clicking the macro opens a detail screen.
5. In the macro editor, each card is either `Texto` or `Imagem`; switching type clears the previous content to avoid ambiguous clipboard behavior.
6. In the macro editor, cards can be moved up or down before saving, changing the sequence shown in the detail screen.
7. In the detail screen, each button shows its sequence number plus a truncated text preview or image thumbnail.
8. Preview buttons can be resized vertically to reveal more content while keeping the compact default layout.
9. Custom preview and editor text-box heights are stored in `localStorage` and keyed by each card's stable `label`, so reordering cards does not swap their saved sizes.
10. Clicking a detail button copies the full text or image to the clipboard and shows a centered `Copiado` toast.
...
In macro button payloads, `number` is generated by the backend from the current order. The frontend uses `label` as a stable per-card key so resized preview and editor text-box heights can remain attached to the same content after reorder operations. API clients may provide any stable non-empty `label`; if omitted, the backend falls back to the generated number.

Each button has a `content_type`:

- `text`: `message` stores plain text.
- `image`: `message` stores a data URL such as `data:image/png;base64,...`. The web UI accepts image files up to 600 KB to keep Firestore documents safely below their size limit.
...
curl -b cookies.txt \
  ...
  -d '{
    "name": "Greeting",
    "buttons": [
      { "label": "greeting-main", "message": "Hello! How can I help you today?" }
    ]
  }' \
  "$BASE_URL/api/macros"
```
...
curl -b cookies.txt \
  ...
  -d '{
    ...
    "buttons": [
      { "label": "restart-app", "message": "Please restart the application and try again." },
      { "label": "incognito-check", "message": "Please confirm whether the issue also happens in an incognito window." },
      { "label": "escalation-note", "message": "I will escalate this with the details collected so far." }
    ]
  }' \
  "$BASE_URL/api/macros"
```

Create an image macro:
    ...
    "buttons": [
      {
        "label": "ready-image",
        "content_type": "image",
        "message": "data:image/png;base64,REPLACE_WITH_BASE64_IMAGE"
      }
    ]
  }' \
  "$BASE_URL/api/macros"
...
curl -X PUT -b cookies.txt \
  ...
  -d '{
    ...
    "buttons": [
      { "label": "restart-app", "message": "Please restart the application and try again." },
      { "label": "screenshot-request", "message": "Please send a screenshot of the error message." }
    ]
  }' \
