...
8. Custom preview and editor text-box heights are stored in `localStorage` and keyed by each text card's stable `label`, so reordering cards does not swap their saved sizes.
9. Clicking a message button copies the full text to the clipboard and shows a centered `Copiado` toast.
...
In macro button payloads, `number` is generated by the backend from the current order. The frontend uses `label` as a stable per-text key so resized preview and editor text-box heights can remain attached to the same message after reorder operations. API clients may provide any stable non-empty `label`; if omitted, the backend falls back to the generated number.
